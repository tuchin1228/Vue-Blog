<template>
  <div class="userinfo ">
    <div v-if="editRole == true" class="editbox">
      <v-btn v-if="!editing" tile color="success" depressed @click="editing = true">
        <v-icon left>
          mdi-pencil
        </v-icon>
        編輯
    ...
    <div v-if="!editing" class="infobox">
      <div class="infoimgbox">
        <img v-if="this.$store.state.Userinfo.avator" :src="this.$store.state.Userinfo.avator" />
        <img
      ...
      <div class="infocontent">
        ...
        <h3>
          <span>興趣</span><small>{{ this.$store.state.Userinfo.interest }}</small>
        </h3>
        <h3>
          <span>專長</span><small>{{ this.$store.state.Userinfo.spec }}</small>
        </h3>
      </div>
    </div>
    <h4 v-if="!editing" class="ownpost">
      最近Po文
    </h4>
    <div v-if="!editing" class="ArticleList" style="margin:0;">
      <ArticleList :articlelist="this.$store.state.ArticleList.data" />
    </div>
    <form class="edit_area" v-if="editing">
      ...
      <section class="edit_confirm">
        ...
        <v-btn v-if="editing" depressed color="error" @click="editing = false">
          取消
        </v-btn>
      </section>
    </form>
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$http
        .put(`${process.env.VUE_APP_baseUrl}/auth/userinformation/${this.$route.params.id}`, {
          newInfo: vm.newInfo,
          authtoken,
        })
        .then((res) => {
